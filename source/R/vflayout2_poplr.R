vflayout2_poplr <- function( vf, grp = 3, nperm = 5000,
...
# Initialize to 1 
  ...
  {  
    nonSeenLocations[which( vf[idx1[i], locini:ncol( vf )] > 0 )] <- 0
  }
...
# sensitivity plot last n visits
  ...
  vf1[which( vf1 < 0 )] <- "<0"
  vfplotloc2( vf1, eye = vfinfo1$seye, patternMap = locmap , outerColor = color, bs = c( vfinfo1$sbsx, vfinfo1$sbsy ), 
             txtfont = ffamilyvf, pointsize = pointsize,
             xminmax = xminmax, yminmax = yminmax,
             outerSymbol = outerSymbol, innerSymbol = innerSymbol,
             outerInch = outerInch, innerInch = innerInch,
...
# PoPLR plot
  ...
  vfplot2_poplr( pres$sl, pres$pval, pres$vfdata,
                txtfont = ffamilyvf, pointsize = pointsize,
                xminmax = xminmax, yminmax = yminmax,
                outerSymbol = outerSymbol, innerSymbol = innerSymbol,
...
# color-code map
  ...
  colormapgraph2( ncol = 3, mapval = colorScale, notSeenAsBlack = TRUE, txtfont = ffamilyvf, pointsize = pointsize,
                 outerSymbol = outerSymbol, innerSymbol = innerSymbol,
                 outerInch = outerInchpoplr, innerInch = innerInchpoplr )
  par( opar )
######################################################

vfplot2_poplr <- function( sl, pval, vfinfo, newWindow = FALSE, txtfont = "mono",
                          pointsize = 7, width = 6,
...
# calculate years to blindness
    sens                     <- as.numeric( vfinfo[locini:ncol( vfinfo ) ] )
    sens[idxNotSeen]         <- 0
    yearstoblind             <- NULL
...
# slope is in dB per 10 years
  ...
  vfplotloc2( sl, eye = vfinfo$seye, patternMap = patternMap,
